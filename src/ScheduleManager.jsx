...
function extractLecturers(raw) {
  ...
  if (matches.length > 0) return matches.map(m => m[1].replace(/\s*[-–]\s*$/,"").trim()).filter(Boolean);
...
function processSheet(rawRows, sheetType) {
  ...
  for (let i=1;i<rawRows.length;i++) {
    ...
    const list = lecturers.length>0?lecturers:[""];
    const shared = {
    ...
    for (const lecturer of list) {
      for (let j=0;j<hdrs.length;j++) {
        ...
        seen.add(dedup);
        result.push({ id:`${sheetType}-${i}-${j}-${encodeURIComponent(lecturer)}`, lecturer, hasLecturer:!!lecturer, ...shared, date:val, time, sessionType:getSessionType(hdrs[j]), hasRoom:!!shared.room });
      }
    }
  }
  return result;
  ...
  const handleFile = useCallback(e => {
    ...
    reader.onload = evt => {
      try {
        ...
        setRows(all); setClashes(detectClashes(all)); setWarnings(getWarnings(all));
